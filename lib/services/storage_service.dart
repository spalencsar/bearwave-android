...
class StorageService {
  ...
  /// Re-read prefs from disk. Required for Android Auto / MediaBrowser: the
  /// audio handler often keeps a stale in-memory cache after the UI wrote data.
  Future<void> reload() async {
    try {
      await _prefs.reload();
    } catch (_) {
      // If the handle was never initialized, re-bind.
      _prefs = await SharedPreferences.getInstance();
    }
  }
  ...
  Future<List<RadioStation>> loadFavorites() async {
    await reload();
    return _parseStationList(_prefs.getString(_favoritesKey));
  }
  ...
  Future<void> recordRecentStation(RadioStation station) async {
    ...
    recent.removeWhere(
      (s) =>
          ...
          (s.uuid == null &&
              (s.urlResolved == station.urlResolved || s.url == station.url)),
    );
  ...
  List<RadioStation> _parseStationList(String? data) {
    ...
    try {
      ...
      return jsonList
          .map((entry) {
            try {
              return RadioStation.fromStorageJson(
          ...
          .where(
            (s) =>
                s.name.isNotEmpty &&
                (s.urlResolved.isNotEmpty || s.url.isNotEmpty),
          )
          .toList();
    } catch (_) {
      return [];
